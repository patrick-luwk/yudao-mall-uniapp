<!-- 欢乐颂 - 欢乐颂 -->
<template>
  <s-layout class="wallet-wrap" title="欢乐颂">
    <!-- 信息统计卡片 -->
    <view class="header-box ss-flex ss-row-center ss-col-center">
      <view class="card-box ui-BG-Main ui-Shadow-Main">
        <view class="card-head ss-flex ss-col-center">
          <view class="card-title ss-m-r-10">当前收益（元）</view>
          <view
    ...
    >
      <view class="ss-p-x-20 ss-p-y-30">
        <view class="model-title ss-m-b-30 ss-m-l-20">新建大路</view>
        <view class="model-subtitle ss-m-b-100 ss-m-l-20">请输入相关信息</view>
        <view class="input-box ss-flex ss-col-center border-bottom ss-m-b-70 ss-m-x-20">
          <view class="unit">桌号</view>
          <uni-easyinput
            :inputBorder="false"
        ...
        <view class="input-box ss-flex ss-col-center border-bottom ss-m-b-70 ss-m-x-20">
          <view class="unit">大路</view>
          <uni-easyinput
            :inputBorder="false"
            class="ss-flex-1 ss-p-l-10"
            v-model="state.record"
            type="number"
            placeholder="请输入正确的14-20位的初始数据"
          />
        </view>
        <button
    ...
    <view v-if="state.pagination.total > 0">
      ...
      >
        <view class="list-content">
          ...
          @click="sheep.$router.go('/pages/user/baijia/baijia-box', { id: item.id })">
            <text class="title ss-line-1">{{ item.tableId }}</text>
            <view class="money">
...
<script setup>
  ...
  const tabMaps = [
    {
      name: '大路提醒',
      value: '1', // BigroadStatusEnum.WAIT_SETTLEMENT
    },
    {
      name: '清算结果',
      value: '2', // BigroadStatusEnum.SETTLEMENT
    },
  ];
  ...
  async function getLogList() {
    ...
    let { code, data } = await BigroadApi.getBigroadPage({
  ...
  async function onConfirm() {
    uni.showModal({
      ...
      success: async function (res) {
        ...
        const { code } = await BigroadApi.createBigroad({
          tableId: state.tableId, //  桌号
          record: state.record,
        });
        if (code === 0) {
  ...
  async function getAgentInfo() {
    const { code, data } = await BigroadApi.getBigroadUserSummary();
    if (code !== 0) {
      return;
    }
    state.summary = data;
